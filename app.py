...
# Tabs for multi-timeframe
tabs = st.tabs(["Daily", "1H", "15M", "5M"])
with tabs[0]:
    st.write("Daily Bias Determination")
    st.line_chart(df_daily.set_index('timestamp')[['close', 'ema9', 'ema21']])
with tabs[1]:
    st.write("1H Structure & Supply/Demand Zones (Coming)")
with tabs[2]:
    st.write("15M EMA(9/21) + RSI Confirmation (Coming)")
with tabs[3]:
    st.write("5M Entry Trigger (Coming)")
...
st.caption("Built with Streamlit • Strategy in progress")
